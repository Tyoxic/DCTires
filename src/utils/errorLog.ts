...
function persist(): void {
  try {
    ...
    const data: PersistedLog = { sessionStartedAt, errors: errorBuffer, breadcrumbs: breadcrumbBuffer };
...
export function addBreadcrumb(category: string, message: string, data?: Record<string, unknown>): void {
  breadcrumbBuffer.push({ category, message, timestamp: new Date().toISOString(), data });
...
function loadPreviousSession(): void {
  try {
    ...
    if (parsed.sessionStartedAt !== sessionStartedAt) previousSession = parsed;
  } catch {
...
export function getRecentErrors(): LogEntry[] { return [...errorBuffer]; }
export function getBreadcrumbs(): Breadcrumb[] { return [...breadcrumbBuffer]; }
export function getPreviousSession(): PersistedLog | null { return previousSession; }
